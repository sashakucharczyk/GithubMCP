...
into a canonical form. The skill is useful for cleaning data sets where the
same entity appears in multiple inconsistent ways.

It is currently designed to do two things:

1 - Review a series of entries and match them to a known value. Useful if you know the full set of possible correct entries.

2 - Given a classification, review a series of entries and clean them to what they best represent under that type. Aim to unify various spellings and formattings into a single entry. (ex: feta + company = Meta, but Meta + food = feta)
